...
export default class CreditApplicationPageContainer extends NavigationMixin(LightningElement){
    ...
    getStateParameters(currentPageReference) {
        if (currentPageReference) {
            console.log('currentPageReference::'+JSON.stringify(currentPageReference.attributes.opptid));
...
0
        ...
        if (data) {
            ...
            if ((this.salesRepList.length === 0) && !this.isLoadedQuote) {
                getSalesRepsFromReturnedOSID({osidJSON:  JSON.stringify(osidObj)})
                    ...
                    this.loading = false;
                    this.showToast('DDT2 Something went wrong', error.body.message, 'error');
            ...
            this.showToast('DDT3 Something went wrong', error.body.message, 'error');
    ...
    handleChangeLocation(event) {
        ...
        getSalesReps({ originatingSiteId: event.target.value })
            ...
            .catch(error => {
                this.loading = false;
                this.showToast('DDT1 Something went wrong', error.body.message, 'error');
            });
    }
