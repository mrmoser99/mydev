...
export default class LookupSearchPanel extends LightningElement {
    ...
    set isdisabled(value){
        ...
        if(value){
            this.crossBtnStyle = 'z-index: -1;';
        }else{
            this.crossBtnStyle = 'z-index: 0;';
        }
    }
    ...
    /** to show lookup field value on load of page */
    @api
    setLookupFieldOnLoad(results){
        this._curSelection = results;    
    }

    get heightDynamically(){

        //return 'height: '+this.searchResultsLocalState.length * 35+'px;overflow: auto;';
        return 'height: 110px;overflow: auto;';
    }


    /** Set search result object to display the list of options under search input */
    @api
    ...
    /** Method checks search term and dispatches event with parameters such as search term, id and object key to the parent*/ 
    updateSearchTerm(newSearchTerm) {
        this._searchTerm = newSearchTerm;

        // Compare clean new search term with current one and abort if identical
        const newCleanSearchTerm = newSearchTerm.trim().replace(/\*/g, '').toLowerCase();
        if (this._cleanSearchTerm === newCleanSearchTerm && newSearchTerm != '') {
            return;
        }
    ...
    /** Method sets search result and dispatch event when user select option from the list */
    processSelectionUpdate(isUserInteraction) {
        ...
        const selectedName = this._curSelection.map((sel) => sel.title);
        const productType = this._curSelection.map((sel) => sel.productType);
        ...
        if (isUserInteraction) {
            const selectEvent = new CustomEvent('selectionchange', {
                detail: {value: selectedIds, selectedName: selectedName, productType: productType}
            });
    ...
    /** Method checks is selection allowed and call the method to search by input value */
    handleInput(event) {
    ...
    /** Method handles keyboard events checking keyCode attribute associated with the ley pressed */
    handleKeyDown(event) {
    ...
    /** Method sets new current selection from the list of the options and calls the method which notifies parent component about changes */
    ...
    /** When user pressing a mouse button over the option from list */
    ...
    /** When user releases a mouse button over the option from list */
    ...
    /** When  the search input gets focus */
    handleFocus() {
        // Prevent action if selection is not allowed
        if (!this.isSelectionAllowed()) {
            return;
        ...
        // update search term with empty value to display all available options
    ...
    /** Method removes selected item. Applies if multi-selection is allowed */
    ...
    /** Method clears current selection value and input search */
    @api
    handleClearSelection(isClear) {
        this._curSelection = [];
        this._hasFocus = false;
        // Process selection update
        this.processSelectionUpdate(true);
